...
def train(net,train_iter,valid_iter,device,num_epcohs):
    net.train()
...
net = text_cnn(vocab_size,weight_matrix,embedding_dim,n_filters,filters_sizes,sentence_max_len,output_dim,dropout).to(device)
optimizer = torch.optim.Adam(net.parameters(),lr=lr)

#train_loss, valid_loss = train(net,train_iter,valid_iter,device,num_epochs)
pred = prediction(net,test_iter)
print(pred)

with open("prediction.txt",'w',encoding = 'utf8') as f:
    for line in pred:
        for l in line:
            f.write(str(l)+'\n')
